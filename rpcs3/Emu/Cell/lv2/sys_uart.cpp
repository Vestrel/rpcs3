...
error_code sys_uart_receive(ppu_thread &ppu, vm::ptr<void> buffer, u64 size, u32 flag)
{
	sys_uart.trace("sys_uart_receive(buffer=*0x%x, size=0x%llx, flag=0x%x)", buffer, size, flag);
...
error_code sys_uart_send(ppu_thread &ppu, vm::cptr<void> buffer, u64 size, u32 flags)
{
